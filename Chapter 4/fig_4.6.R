...
mkCommonPaths <- function(m) {
  list(
    mxPath("res", paste0('re',rev(1:2)),
           values=1, free=c(F,T),
           labels=paste0('res_by_re',rev(1:2))),
    mxPath("har", paste0('hw',1:3),
           values=1, free=c(F,T,T),
           labels=paste0('har_by_hw',1:3)),
...
mxEval(skl_on_pay - skl_bon_pay, skl) # betac1
mxEval(skl_on_res - skl_bon_res, skl) # betac2
mxEval(skl_on_har - skl_bon_har, skl) # betac3
mxEval(har_on_res - har_bon_res, skl) # betac4
